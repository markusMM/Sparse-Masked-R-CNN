"""
Sparse Mask R-CNN
The main Mask R-CNN model implemenetation with sparsified training.
"""
...
import numpy as np
import sparse_mask_rcnn.config
import sparse_mask_rcnn.utils.process
import torch
...
from sparse_mask_rcnn.utils import visualize
from nms.nms_wrapper import nms
from roialign.roi_align.crop_and_resize import CropAndResizeFunction
import sparse_mask_rcnn.utils.detection
from sparse_mask_rcnn.utils.process import mold_image
from sparse_mask_rcnn.utils.process import compose_image_meta
from sparse_mask_rcnn.utils.process import parse_image_meta
from sparse_mask_rcnn.config import Config
...
def param_l2_norm(W, use_sigma: bool = True):
    if use_sigma:
        sigma = W.var()
    else:
        sigma = 1
    return ((W - W.mean())**2).sum().sqrt() / sigma

def param_l1_norm(W, use_sigma: bool = True):
    if use_sigma:
        sigma = W.std()
    else:
        sigma = 1
    return (W - W.mean()).abs().sum() / sigma

def net_norm(
    net: nn.Module, 
    norm_fcn: Callable[[nn.Parameter, bool], torch.Tensor] = param_l2_norm,
    use_sigma: bool = True,
    normalize: bool = True
) -> torch.Tensor:
    nom = torch.tensor(1e-8)
    den = torch.tensor(1 + 1e-8)
    for name, W in net.named_parameters():
        if 'weight' in name:
            norm = norm_fcn(W, use_sigma)
            nom += norm
            if normalize:
                den *= norm
    return nom / den
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if torch.nonzero(positive_roi_bool).size():
        ...
        deltas = Variable(sparse_mask_rcnn.utils.detection.box_refinement(positive_rois.data, roi_gt_boxes.data), requires_grad=False)
    ...
    if positive_count > 0 and negative_count > 0:
        rois = torch.cat((positive_rois, negative_rois), dim=0)  # type: ignore
        zeros = Variable(torch.zeros(negative_count), requires_grad=False).int()
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        roi_gt_class_ids = torch.cat([roi_gt_class_ids, zeros], dim=0)  # type: ignore
        zeros = Variable(torch.zeros(negative_count,4), requires_grad=False)
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        deltas = torch.cat([deltas, zeros], dim=0)  # type: ignore
        zeros = Variable(torch.zeros(negative_count,config.MASK_SHAPE[0],config.MASK_SHAPE[1]), requires_grad=False)
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        masks = torch.cat([masks, zeros], dim=0)
    elif positive_count > 0:
        rois = positive_rois  # type: ignore
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    image, window, scale, padding = sparse_mask_rcnn.utils.process.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    mask = sparse_mask_rcnn.utils.detection.resize_mask(mask, scale, padding)
    ...
    bbox = sparse_mask_rcnn.utils.detection.extract_bboxes(mask)
    ...
    if use_mini_mask:
        mask = sparse_mask_rcnn.utils.detection.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        # Compute overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = sparse_mask_rcnn.utils.detection.compute_overlaps(anchors, crowd_boxes)
    ...
    overlaps = sparse_mask_rcnn.utils.detection.compute_overlaps(anchors, gt_boxes)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset, config, augment=True):
        ...
        self.anchors = sparse_mask_rcnn.utils.detection.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
...
class MaskRCNN(nn.Module):
    ...
    def __init__(
        ...
        model_dir: str, 
        mode: str = "train",
        sparsify_rpn: bool = False,
        sparsify_clf: bool = True,
        sparsify_msk: bool = False,
        sparsify_fpn: bool = True
    ) -> None:
        """
        ...
        if "train" in mode:
            ...
            self.train(False)
        self.sparsify_rpn = sparsify_rpn
        self.sparsify_clf = sparsify_clf
        self.sparsify_msk = sparsify_msk
        self.sparsify_fpn = sparsify_fpn
    ...
    def build(self, config: Config) -> None:
        ...
        # Backbone
        # Bottom-up Layers (shared convolution)
        ...
        self.anchors = Variable(torch.from_numpy(
            sparse_mask_rcnn.utils.detection.generate_pyramid_anchors(
    ...
    def find_last(self):
        ...
        key = self.config.NAME.lower()  # type: ignore
    ...
    def detect(self, images):
        ...
        detections, mrcnn_mask = self.predict([molded_images, image_metas], mode='inference')  # type: ignore
    ...
    def predict(self, input, mode):
        ...
        rpn_rois = proposal_layer([rpn_class, rpn_bbox],  # type: ignore
    ...
    def train_model(
        ...
        l2_norm: Allows to use an additional L2-norm on the weights for the loss.
        l1_norm: Allows to use an additional L1-norm on the weights for the loss.
        use_sigma: Whether the corresponding norm part of the loss shall be divided
                   by the VAR or STD.
        """
        ...
        if layers in layer_regex.keys():
            layers = layer_regex[layers]  # type: ignore
        ...
        for epoch in range(self.epoch+1, epochs+1):
            ...
            (loss, loss_rpn_class, loss_rpn_bbox, 
             loss_mrcnn_class, loss_mrcnn_bbox, loss_mrcnn_mask) = self.train_epoch(
                train_generator, 
                optimizer, 
            ...
            (val_loss, val_loss_rpn_class, val_loss_rpn_bbox, 
             val_loss_mrcnn_class, val_loss_mrcnn_bbox, val_loss_mrcnn_mask) = self.valid_epoch(
    ...
    def train_epoch(
        ...
        for inputs in datagenerator:
            ...
            rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask = \
                self.predict([images, image_metas, gt_class_ids, gt_boxes, gt_masks], mode='training')  # type: ignore
            ...
            if l2_norm:
                l2_losses = dict(
                    fpn_l2_loss = torch.tensor(0) if not self.sparsify_fpn 
                    else net_norm(self.fpn, param_l2_norm, use_sigma),
                    rpn_l2_loss = torch.tensor(0) if not self.sparsify_rpn 
                    else net_norm(self.rpn, param_l2_norm, use_sigma),
                    clf_l2_loss = torch.tensor(0) if not self.sparsify_clf 
                    else net_norm(self.classifier, param_l2_norm, use_sigma),
                    msk_l2_loss = torch.tensor(0) if not self.sparsify_msk 
                    else net_norm(self.mask, param_l2_norm, use_sigma),
                )
                loss += sum(l2_losses.values())
                self.l2_losses = l2_losses  # "log" L2 losses for later

            if l1_norm:
                l1_losses = dict(
                    fpn_l1_loss = torch.tensor(0) if not self.sparsify_fpn 
                    else net_norm(self.fpn, param_l1_norm, use_sigma),
                    rpn_l1_loss = torch.tensor(0) if not self.sparsify_rpn 
                    else net_norm(self.rpn, param_l1_norm, use_sigma),
                    clf_l1_loss = torch.tensor(0) if not self.sparsify_clf 
                    else net_norm(self.classifier, param_l1_norm, use_sigma),
                    msk_l1_loss = torch.tensor(0) if not self.sparsify_msk 
                    else net_norm(self.mask, param_l1_norm, use_sigma),
                )
                loss += sum(l1_losses.values())
                self.l1_losses = l1_losses  # "log" L1 losses for later
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = sparse_mask_rcnn.utils.process.resize_image(
                image,
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        for i in range(N):
            ...
            full_mask = sparse_mask_rcnn.utils.detection.unmold_mask(masks[i], boxes[i], image_shape)
